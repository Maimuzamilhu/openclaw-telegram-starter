 ...
 * 1. RAG (Retrieval Augmented Generation) - for message history
 ...
 * 3. Merges Telegram tools with MCP tools for the LLM
...
import { getSessionHistory, addMessage, Message } from '../memory/database.js';
import {
  shouldUseRAG,
  retrieve,
  buildContextString,
...
import {
  ...
  scheduleMessage,
  setReminder,
} from '../tools/telegram-actions.js';
 ...
 * The agent knows it has access to message history and external tools.
 */
const SYSTEM_PROMPT = `You are a helpful AI assistant integrated into Telegram.
...
- Telegram tools: search_knowledge_base, send_message, get_channel_history, etc.

## Response Format:
- Be concise
- Use Telegram Markdown formatting: *bold*, _italic_, \`code\`
- For GitHub/Notion results, format nicely with links`;

/**
 * Telegram tool definitions for OpenAI function calling.
 * These are the built-in Telegram tools.
 */
const TELEGRAM_TOOLS: OpenAI.Chat.ChatCompletionTool[] = [
  {
    type: 'function',
    function: {
      name: 'search_knowledge_base',
      description: 'PRIORITY TOOL: Search through indexed message history using semantic search. Use this for ANY question about past discussions, topics, decisions, or finding what was said.',
      parameters: {
        type: 'object',
        properties: {
          query: {
          ...
          channel_name: {
            type: 'string',
            description: 'Optional: limit search to a specific channel or group name',
          },
          limit: {
            type: 'number',
  ...
  },
  {
    ...
    function: {
      name: 'send_message',
      description: 'Send a message to a Telegram user or group immediately',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Get recent messages from a Telegram group. Note: Telegram bots have limited history access. For questions about past discussions, use search_knowledge_base instead.',
      parameters: {
        type: 'object',
        properties: {
          channel_name: { type: 'string', description: 'Group name or chat ID' },
  ...
  },
  {
    ...
    function: {
      name: 'list_channels',
      description: 'List all known Telegram groups the bot is in',
  ...
  },
  {
    ...
    function: {
      name: 'list_users',
      description: 'List all known users who have interacted with the bot',
      parameters: { type: 'object', properties: {} },
    },
  },
  // ============== MEMORY TOOLS ==============
...
function getAllTools(): OpenAI.Chat.ChatCompletionTool[] {
  ...
  if (isMCPEnabled()) {
    ...
    logger.info(`Total tools available: ${allTools.length} (${TELEGRAM_TOOLS.length} Telegram + ${openAIMcpTools.length} MCP)`);

    // Log MCP tool names for debugging
    const mcpToolNames = mcpTools.map(t => t.name).join(', ');
    logger.debug(`MCP tools: ${mcpToolNames}`);
  } else {
    logger.debug(`MCP not enabled, using ${TELEGRAM_TOOLS.length} Telegram tools only`);
  }
  ...
  try {
    switch (name) {
      ...
      case 'search_knowledge_base': {
        ...
        // Try to resolve group name if provided
        if (channelNameFilter) {
          const channel = await findChannel(channelNameFilter);
          if (channel) {
            channelNameFilter = channel.name;
            logger.info(`Resolved group: ${channelNameFilter}`);
          }
          // Remove # prefix if present
          channelNameFilter = channelNameFilter.replace(/^#/, '').trim();
        }
 ...
 * 3. Retrieves relevant message history if needed
...
export async function processMessage(
  ...
  if (ragContext) {
    messages.push({
      role: 'system',
      content: `The following context from message history may be relevant to the user's question:\n\n${ragContext}`
    });
  }
...
export async function summarizeThread(
  ...
  const summaryPrompt = `Please provide a concise summary of this conversation thread. Focus on:
