 ...
 * Retriever Module
 * 
 * This module handles semantic search over the indexed messages.
...
export function parseQueryFilters(query: string): {
  ...
  const channelMatch = query.match(/#(\S+)/);
  if (channelMatch) {
